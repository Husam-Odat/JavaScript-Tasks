...
function areEquivalentObjects(obj1, obj2) {
  ...
  // Example usage:
  const obj1 = { name: 'John', age: 27, city: 'New York' };
  const obj2 = { name: 'John', age: 27, city: 'New York' };
  const obj3 = { name: 'Alice', age: 25, city: 'Los Angeles' };
  
  console.log(areEquivalentObjects(obj1, obj2)); // Output: true
  console.log(areEquivalentObjects(obj1, obj3)); // Output: false
...
function copyToClipboard(text) {
  ...
  // Example usage:
//   const textToCopy = 'dddddvcvb';
  copyToClipboard('husam');
...
function objectsToCSV(arrayOfObjects, columns) {
    const csvColumns = columns.join(',');
    const csvData = arrayOfObjects.map(obj =>
      columns.map(column => obj[column])+(',')
    )+('\n');
  ...
  const selectedColumns = ['ID', 'Name', 'City'];
  const csvString2 = objectsToCSV(data, selectedColumns);
